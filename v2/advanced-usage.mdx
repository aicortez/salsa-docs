...
Integrate a wide range of sensors for comprehensive monitoring:
- **Temperature Sensors**: Monitor salsa storage conditions
- **RFID Tags**: Track ingredient usage and inventory movement
- **Vibration Sensors**: Detect equipment performance issues
- **Humidity Sensors**: Ensure optimal storage environments
...
sensors.forEach(sensor => {
  ...
  if (reading.isAnomaly()) {
    console.log(`⚠️  Alert: ${reading.anomalyDetails}`);
  }
});
// Query historical data
const history = await salsa.iot.queryHistory({
...
```php
// Example: Query sensor data via API
$salsa = new SalsaClient(['apiKey' => 'your_api_key']);
// Get real-time sensor readings
$sensors = $salsa->iot->getSensors(['location' => 'kitchen_1']);
foreach ($sensors as $sensor) {
    $reading = $sensor->getLatestReading();
    echo "{$sensor->name}: {$reading->value} {$reading->unit}\n";
    if ($reading->isAnomaly()) {
        echo "⚠️  Alert: {$reading->anomalyDetails}\n";
    }
}
// Query historical data
$history = $salsa->iot->queryHistory([
    'sensor_id' => 'temp_sensor_001',
    'start_date' => '2024-01-01',
    'end_date' => '2024-01-31',
    'aggregation' => 'hourly'
...
## Best Practices
- Regularly review and update your AI forecasting configuration for optimal accuracy.
- Continuously monitor sensor data and set up alerts for any anomalies.
- Use automation features to reduce manual workload and improve efficiency.
- Leverage enterprise features for better control and management of multi-location operations.
- Always test new configurations and workflows in a staging environment before deploying to production.
